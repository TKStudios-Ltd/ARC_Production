  ...
  theme.classes.FrameworkRadios = (function() {
    ...
    FrameworkRadios.prototype.updateLabelOnChange = function() {
      ...
      return _this.inputs.on('change', function() {
        var valll = el(this).val();
        document.querySelectorAll('.color_images').forEach( e => {
          e.classList.remove("active");
        });
        document.querySelectorAll('.color_images_carousel').forEach( e => {
          e.classList.remove("active");
        });
        document.querySelector('.color_images[data-color="'+valll+'"]').classList.add('active');
        document.querySelector('.color_images_carousel[data-color="'+valll+'"]').classList.add('active');
        var elem = document.querySelector('.color_images.active .product-page--grid');
        ...
        document.querySelectorAll('.color_images_carousel .carousel--y-container').forEach( e => {
          e.style.height = 0;
        });
//         document.querySelectorAll('.color_images_carousel .carousel--x-container').forEach( e => {
//           e.style.transform = "translateX(0)";
//         });
//         document.querySelectorAll('.color_images_carousel .carousel--dot-nav .carousel--dot').forEach( e => {
//           e.dataset.active = "false";
//         });
//         document.querySelector('.color_images_carousel.active .carousel--dot-nav').firstChild.dataset.active = "true";
        var tallest = document.querySelector('.color_images_carousel.active .carousel--y-container .carousel--block').offsetHeight;
        document.querySelector('.color_images_carousel.active .carousel--y-container').style.height = tallest + "px";
        var root_new = document.querySelector('.color_images_carousel.active .carousel--root').offsetWidth;
        var dot_nav = document.querySelector('.color_images_carousel.active .carousel--dot-nav').offsetWidth;
        var trans = (root_new - dot_nav) / 2;
        document.querySelector('.color_images_carousel.active .carousel--dot-nav').style.transform = "translateX(" + trans+ "px)";
    ...
    "athleticgrey": "#a7adb1",
    "terrecotta": "#cc4c31",
    "navy/charcoal": "//cdn.shopify.com/s/files/1/0581/0936/2359/files/navy-charcoal_100x.png?v=18366252873129390148",
    "black/charcoal": "//cdn.shopify.com/s/files/1/0581/0936/2359/files/black-charcoal_100x.png?v=17035640978556645343"
};
      ...
      request.onload = function() {
        ...
        if (request.status >= 200 && request.status < 300) {
          ...
          if (type === 'product') {
            ...
            var product = document.querySelectorAll('.modal--content.active .search--products .product--root');
            var visible = [];
            for ( var i = 0; i < product.length; i++) {
              if (product[i].offsetParent !== null){
                visible.push(product[i]);
              }
            }
            var product_count = visible.length;
            document.querySelector('.search--count>a').innerHTML = 'Products '+product_count;
            if (product_count > 0){
              document.querySelector('.search--no-products').style.display = 'none';
              document.querySelector('.search--view-more').style.display = 'block';
            }
            else{
              document.querySelector('.search--no-products').style.display = 'block';
              document.querySelector('.search--view-more').style.display = 'none';
            }
            //    Password Page end
  ...
  theme.classes.Product = (function(superClass) {
    ...
    function Product(root) {
      ...
      if (_this.root.find('.product-page--left-column').el[0].classList.contains('color_images')) {
        _this.grid = _this.root.find('.color_images.active .product-page--grid');
      } else {
...
(function(i,n){if(typeof define=="function"&&define.amd){define(["flickity/js/index","imagesloaded/imagesloaded"],function(t,e){return n(i,t,e)})}else if(typeof module=="object"&&module.exports){module.exports=n(i,require("flickity"),require("imagesloaded"))}else{i.Flickity=n(i,i.Flickity,i.imagesLoaded)}})(window,function t(e,i,s){"use strict";i.createMethods.push("_createImagesLoaded");var n=i.prototype;n._createImagesLoaded=function(){this.on("activate",this.imagesLoaded)};n.imagesLoaded=function(){if(!this.options.imagesLoaded){return}var n=this;function t(t,e){var i=n.getParentCell(e.img);n.cellSizeChange(i&&i.element);if(!n.options.freeScroll){n.positionSliderAtSelected()}}s(this.slider).on("progress",t)};return i});
  ...
  var flkty_elements = document.querySelectorAll('.featured-collection--grid');
  flkty_elements.forEach(function(elem){
  ...
  window.onload = function(){
    root.style.setProperty("--announcemet-height", document.querySelector('.announcement--root').clientHeight + 'px');
    root.style.setProperty("--header-height", document.querySelector('.header--root').clientHeight + 'px');
    if(document.body.contains(document.querySelector('.contact-page--root'))){
      document.querySelector('.contact-page--left-column').style.height=document.querySelector('#main-content').style.minHeight;
    }
  }
  ...
  var original_pswd = CryptoJS.AES.encrypt('athletiquerecreation', "Ngodeinweb");
    document.body.setAttribute('original',original_pswd.toString());
	if (document.querySelector('#col_sub')){
      document.querySelector('#col_sub').addEventListener('click',function(e){
        e.preventDefault();
        var f = true;

        var old_pswd = CryptoJS.AES.decrypt(document.body.getAttribute('original'), "Ngodeinweb"); 
        var pswd = document.querySelector('#coll_pswd');

        if(pswd.value.trim() == '' || pswd.value != old_pswd.toString(CryptoJS.enc.Utf8)) {
          if(pswd.value == ''){
            var message = "Please Enter password";
          }else {
            message = "Invalid Password";
          }
          document.querySelector('.col-form-error').innerHTML= message;
          document.querySelector('.col-form-error').style.display = "block";

          f = false;
          e.preventDefault();
        } else {
          document.querySelector('.col-form-error').innerHTML= '';
          document.querySelector('.col-form-error').style.display = "none";
        } 
...
var product = document.querySelectorAll('.search--root .product--root');
            var visible = [];
            for ( var i = 0; i < product.length; i++) {
              if (product[i].offsetParent !== null){
                visible.push(product[i]);
            ...
            var product_count = visible.length;
            if(document.querySelector('.search-count')){
              document.querySelector('.search-count').innerHTML = product_count;
            }
            if(document.querySelector('.search-result-empty-text')){
              if (product_count > 0){
                document.querySelector('.search-result-empty-text').style.display = 'none';
              }
              else{
                document.querySelector('.search-result-empty-text').style.display = 'block';
              }
            }
